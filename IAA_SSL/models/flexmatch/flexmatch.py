...
from ssl_utils import ce_loss, wd_loss, EMA, Bn_Controller, EDMLoss

from sklearn.metrics import *
from copy import deepcopy

class classifiy_MLP(nn.Module):
    def __init__(self, in_dim, hidden_dim=1024, out_dim=10):
        super().__init__()
...
class main_model(nn.Module):
    def __init__(self, backbone, output):
        ...
        self.cl = classifiy_MLP(backbone.output_dim, out_dim=output)
    def forward(self, x):
        x = self.backone(x)
        return self.cl(x)

class FlexMatch:
    def __init__(self, backbone, num_classes, ema_m, T, p_cutoff, lambda_u, \
        ...
        Args:
            backbone: backbone network class (see net_builder in utils.py)
            ...
            tb_log: tensorboard writer (see ssl_utils.py)
        ...
        # other configs are covered in pretrain_main.py

        self.model = main_model(backbone, num_classes)
        self.ema_model = None
    ...
    def train(self, args, logger=None):
        ...
        for (x_lb, y_lb), (x_ulb_idx, x_ulb_w, x_ulb_s) in zip(self.loader_dict['train_lb'],
            ...
            pseudo_counter = Counter(selected_label.tolist()) # 全是-1的list，长度为target
            if max(pseudo_counter.values()) < len(self.ulb_dset):  # not all(5w) -1
                # print(max(pseudo_counter.values()), len(self.ulb_dset))
            ...
            # print(x_lb.shape, x_ulb_w.shape, x_ulb_s.shape, inputs.shape)
            ...
            with amp_cm():
                ...
                sup_loss = self.criterion(p_target=y_lb, p_estimate=logits_x_lb)
    ...
    def evaluate(self, eval_loader=None, args=None):
        ...
        for _, x, y in eval_loader:
            ...
            total_num += num_batch
            logits = self.model(x)
            loss = self.criterion(p_target=y, p_estimate=logits)
            # loss = F.cross_entropy(logits, y, reduction='mean')
            # y_true.extend(y.cpu().tolist())
            y_true.extend(torch.max(y, dim=-1)[1].cpu().tolist())
